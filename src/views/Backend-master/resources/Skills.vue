                            ...
                            <v-col cols="12" md="12">
                                <v-hover v-slot:default="{ hover }" open-delay="200">
                                    <v-card :elevation="hover ? 16 : 2" class="mx-auto">
                                        <v-card-text>
                                            <v-data-table :headers="headers" :items="skills" sort-by="calories"
                                                class="elevation-1">
                                                <template v-slot:top>
                                                    <v-toolbar flat>
                                                        <v-toolbar-title>Manage Skills</v-toolbar-title>
...
export default {
    ...
    methods: {
        async initialize() {
            let result = await axios.get(`/skill`);
        ...
        async deleteItemConfirm() {
            console.log(this.editedItem);
            let result = await axios.delete(`/skill/` + this.editedItem.id);
        ...
        async save() {
            if (this.editedIndex > -1) {
                let result = await axios.put(`/skill/` + this.editedItem.id, { 'name': this.editedItem.name });
                console.log(result);
                if (result.status == 200) {
                    this.initialize();
                ...
                let result = await axios.post(`/skill`, { 'name': this.editedItem.name });
                console.log(result);
                if (result.status == 200) {
