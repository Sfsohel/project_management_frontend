                            ...
                            <v-col cols="12" md="12">
                                <v-hover v-slot:default="{ hover }" open-delay="200">
                                    <v-card :elevation="hover ? 16 : 2" class="mx-auto">
                                        <v-card-text>
                                            <v-data-table :headers="headers" :items="designations" sort-by="calories"
                                                class="elevation-1">
                                                <template v-slot:top>
                                                    <v-toolbar flat>
                                                        <v-toolbar-title>Manage Designaitons</v-toolbar-title>
...
export default {
    ...
    methods: {
        async initialize() {
            let result = await axios.get(`/designaiton`);
        ...
        async deleteItemConfirm() {
            let result = await axios.delete(`/designaiton/` + this.editedItem.id);
        ...
        async save() {
            if (this.editedIndex > -1) {
                let result = await axios.put(`/designaiton/` + this.editedItem.id, { 'name': this.editedItem.name });
                console.log(result);
                if (result.status == 200) {
                    Object.assign(this.designations[this.editedIndex], this.editedItem)
                }
            } else {
                let result = await axios.post(`/designaiton`, { 'name': this.editedItem.name });
                if (result.status == 200) {
